...
export class WhatsAppBot {
  ...
  private async handleIncomingMessage(msg: any) {
    ...
    const stickerCommands = ['.stickermenu', 'stickermenu', '.stiker', 'stiker', '.hd', 'hd', '.brat', 'brat', '.bratvid', 'bratvid', '.smeme', 'smeme', '.qc', 'qc', '.toimg', 'toimg', '.togif', 'togif', '.stikerrandom', 'stikerrandom', '.stikerspongebob', 'stikerspongebob', '.iqc', 'iqc', '.tovideo', 'tovideo', '.rvo', 'rvo', '.hdvid', 'hdvid', '.emojimix', 'emojimix', '.emojigif', 'emojigif', '.bratgambar', 'bratgambar'];
...
│ .totalfitur`;
      ...
      if (!jid.endsWith("@g.us")) {
        ...
        const isAdmin = participants.some((p) => p.id === senderId && (p.admin === "admin" || p.admin === "superadmin")) || isOwner;
      ...
      if (!jid.endsWith("@g.us")) {
        ...
        // If no target is specified, check the sender's own warnings
        if (targets.length === 0) {
           targets = [senderId];
        }
...
${e.stack || e.message || String(e)}` }, { quoted: msg });
       ...
       if (!quotedMsg) {
           ...
           try {
               ...
               // Contact names are not always available unless we have a store, but we can try using the pushName if it's the sender, or just use the number
